...
def main():
    ...
    for region in all_regions:
        ...
        relevant_flows = list(
            filter(lambda x: x['FlowLogStatus'] == 'ACTIVE' and x['TrafficType'] == 'ALL' and x['ResourceId'] in vpcs,
                   all_flows_in_region))
        ...
        vpcs_with_s3 = list(set(vpcs_with_s3) - set(
            vpcs_with_cwl))  # do not include in s3 if already in CWL as this test is more accurate
...
def query_vpcfl_daily_size_s3(bucket, trailorflow):
    d = datetime.today() - timedelta(
        days=1)  # today might be partial, we'll take a look at yesterday to have a full day
